# Next.js Microfrontends Example: Yarn Workspace

Yarn workspace로 구성된 마이크로프론트엔드 예제입니다.
...
`yarn workspaces foreach --parallel --interlaced run dev`로 3개 앱을 병렬 실행합니다.
...
```
examples/yarn-workspace/
├── package.json             # yarn workspace 루트
...
| 기능        | Yarn workspace                       | pnpm workspace        | Turborepo + npm           |
| ----------- | ------------------------------------ | --------------------- | ------------------------- |
| 의존성 관리 | Yarn (PnP 또는 node_modules)         | pnpm (심볼릭 링크)    | npm workspaces (호이스팅) |
| 병렬 실행   | `yarn workspaces foreach --parallel` | `pnpm --parallel -r`  | `turbo dev`               |
| 빌드 캐싱   | ❌ 없음                              | ❌ 없음               | ✅ 원격/로컬 캐시         |
| 설정 파일   | `package.json` workspaces            | `pnpm-workspace.yaml` | `turbo.json`              |
